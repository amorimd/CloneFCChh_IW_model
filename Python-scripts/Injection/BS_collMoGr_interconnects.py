...
from FCChh_coll_imp import FCChh_manycoll_iw_model_refine
...
if __name__ == "__main__":
    ...
    root_result='/afs/cern.ch/work/d/damorim/work/DELPHI_results/'+machine+'/BS_collMoGr_interconnects_'+Estr+'V1';
    ...
    collSettingsFileName='collgaps_inj3.3TeV_MoGr.dat'
    ...
    for imat,material in enumerate(materialscan):
        ...
        for ihalfgap,halfgap in enumerate(hgapscan):
            ...
            imp_mod_vac,wake_mod_vac=imp_model_elliptic(iw_input,halfgap,orientation='V',
                    wake_calc=wake_calc,flagrm=True,lxplusbatch=lxplusbatchImp,queue='1nh',dire=machine+'vacpipe_holes_'+Estr+'/');
            ...
            #Lh = 8e-3; Wh = 1.5e-3; T = 1.075e-3; b = halfgap; d = halfgap+3E-3; rhob = 6e-7; rhod = 6e-7;nb_holes_per_cs = 2;eta = nb_holes_per_cs*((Lh-Wh)*Wh+np.pi*(Wh/2)**2)/(2*np.pi*b*2*Lh); fcutoff = 1.8412*c/(2.0*np.pi*halfgap);
            #imp_holes,wake_holes=imp_model_holes_Mostacci(Lh,Wh,T,b,d,
	    #        eta,rhob,rhod,circ,
	    ...
	    #        nb_holes_per_crosssection=nb_holes_per_cs,Cm=1.,Ce=1.)
	        ...
	        comment='LHCcoll',dire='FCChh_BS_collMoGr_interconnects_'+Estr+'/');

            #interconnects: resonator
            fpar2=freq_param(fmin=1.0e9,fmax=1.0e12,ftypescan=2,nflog=500,fminrefine=2.95e9,fmaxrefine=3.15e9,nrefine=200000)
            ...
            add_impedance_wake(imp_mod,imp_interconnects,betax/avbetax,betay/avbetay);
            add_impedance_wake(wake_mod,wake_interconnects,betax/avbetax,betay/avbetay);
